...
function menuhover(menu){
	if(document.defaultView.getComputedStyle(menu.firstElementChild,null).width!="32px"){
		menu.firstElementChild.className="menuhover";
		menu.lastElementChild.className="phover";
	}
}
function menuleave(menu){
	if(document.defaultView.getComputedStyle(menu.firstElementChild,null).width!="32px"){
		menu.firstElementChild.className="menuleaver";
		menu.lastElementChild.className="pleaver";
	}
}
function divclick1(menu){
	if(document.defaultView.getComputedStyle(menu.firstElementChild,null).width!="32px"){
		menu.firstElementChild.className="menuclick";
		menu.lastElementChild.className="pclick";
		menu.nextElementSibling.firstElementChild.className="menucall";
		menu.nextElementSibling.lastElementChild.className="pcall";
		document.getElementById("shangping").className="imgdisplay";
		document.getElementById("hecha").className="imghide";
	}
}
function divclick2(menu){
	if(document.defaultView.getComputedStyle(menu.firstElementChild,null).width!="32px"){
		menu.firstElementChild.className="menuclick";
		menu.lastElementChild.className="pclick";
		menu.previousElementSibling.firstElementChild.className="menucall";
		menu.previousElementSibling.lastElementChild.className="pcall";
		document.getElementById("shangping").className="imghide";
		document.getElementById("hecha").className="imgdisplay";
	}
}
window.onload=function(){
	var menu=document.getElementsByClassName("package")[0];
	menu.firstElementChild.className="menuclick";
	menu.lastElementChild.className="pclick";
	//出场特效
	document.getElementById('rightcontent').style.animation='chuchanggoods 1s linear';
	document.getElementById('leftcontent').style.animation='chuchanggoods 1s linear';
	setTimeout("chuchang()",1000);
}
function shophover(menu){
	menu.firstElementChild.className='shophover';
	setTimeout(function(){
		menu.lastElementChild.className='shopimghover';
	},300);
	
}
function shoplaster(menu){
	menu.lastElementChild.className='shopimgleaver';
	setTimeout(function(){
		menu.firstElementChild.className='shopleaver';
	},400);
}
function chuchang(){
	document.getElementsByClassName("imglogo")[0].style.animation="chuchangimg1 1s linear";
}